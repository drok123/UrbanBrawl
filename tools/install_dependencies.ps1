...
function Validate-Files([string[]]$RelativePaths, [string]$Label) {
    foreach ($relativePath in $RelativePaths) {
        ...
        if (-not (Test-Path $absolutePath)) {
            throw "$Label install is incomplete; required file is missing: $absolutePath"
        }
    }
}
...
foreach ($folder in @("equipment", "inventory", "itemization", "tooltip", "vendor")) {
    ...
    Remove-DirectoryRobust $destination
...
Remove-DirectoryRobust $beehaveDestination
...
Remove-DirectoryRobust $roadDestination
